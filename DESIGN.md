...
**Key Characteristics:**
- Off-white canvas (`#F7F7F5`) as the page background — just warm enough to distinguish from pure white without feeling tinted
- Cards in pure white (`#FFFFFF`) — they lift cleanly off the canvas without heavy shadow
...
### Primary
- **Burgundy** (`#800020`): The left circle of the brand mark — used only in the brand logo, never as a UI color.
...
### Surface & Background
- **Canvas Off-White** (`#F7F7F5`): The page canvas. A barely-there warm off-white — the default body background. Sits just a step below pure white so cards pop without needing heavy shadows.
- **Soft Surface** (`#EFEFED`): One step darker than canvas — used for nested sections, dividers, or hover states that need to feel recessed rather than elevated.
- **White** (`#FFFFFF`): The card surface color. Used for all cards, the floating navigation pill, modal surfaces, secondary button fills, and satellite-CTA circles. Sits visibly above the canvas.
...
- **Priceless Burgundy + Yellow**: The full-color brand logo mark is the only place the brand's burgundy and yellow appear together; they lock the identity to the page without acting as a UI palette.
...
### Do
- Use Canvas Off-White (`#F7F7F5`) as the default body background
- Use pure White (`#FFFFFF`) for all card surfaces — the contrast between canvas and card does the work
...
### Don't
- Don't use the canvas off-white (`#F7F7F5`) as a card color — cards must be pure white to maintain the layering
...
- Background: "Canvas Off-White (`#F7F7F5`) — barely-there warm off-white body canvas, one step below pure white"
- Card surface: "White (`#FFFFFF`) — pure white cards lift cleanly off the off-white canvas"
- Lifted surface: "Soft Surface (`#EFEFED`) — one step darker than canvas for recessed or nested sections"
...
6. Default page backgrounds to Canvas Off-White (`#F7F7F5`), cards to White (`#FFFFFF`) — the separation between the two is the visual foundation of the system
...
- The brand logo mark (burgundy `#800020` + yellow `#F79E1B`) is a brand asset, not a UI palette entry.
